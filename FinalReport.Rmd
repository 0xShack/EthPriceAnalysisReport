...
knitr::include_graphics("Ethereum_logo.png")
```
...
varImpPlot(FinancialModelRF, n.var = 10, main = "Figure 1: Financial Data RF Results")
```
...
Next, we attempt to predict price changes in ETH using only internal data from the Ethereum network. In order to reduce the volatility of these data points, we constructed rolling 3-day and 5-day averages, which have been labeled accordingly. As Figure 3 below demonstrates, gas usage and transaction counts were important indicators for predicting ETH's change in price.
...
title("Figure 4: Ethereum Data Sample Decision Tree", font.main = 1)
```

\pagebreak

Lastly, when network transactions increase, we can see a very obvious increase in the price and vice versa when network activity decreases. Figure 5 below demonstrates this well.

```{r, echo=FALSE, fig.cap="Figure 5: Daily Transactions and Price of Ethereum"}
knitr::include_graphics("transactions.png")
```
...
Finally, we use both datasets combined to predict changes in the price of Ethereum. Again, the performance of competing cryptocurrencies and the performance of the energy sector are important for predicting changes in the price of ethereum. However, we also see that the amount of gas used and the transaction count although less influential, are still important in explaining ETH prices.
...
varImpPlot(CombinedModelRF, n.var = 10, main = "Figure 6: Combined RF Results")
```
...
In order to find the words most associated with ETH, we created a corpus on the textual data contained in each tweet and a data-frame matrix as the foundation for our frequency plot. We find that, barring the word “ethereum”, the most common words in the tweets include “crypto”, “bitcoin”, and "nft", as well as more reactionary words, such as "will", "just", and “now”. As observable from conclusions drawn with the preceding models, changes in BTC price have an effect on the price of ETH as supported by this textual analysis. However, the meaning of the remaining terms have yet to be explored in this analysis and the previous literature reviewed.
...
tweets <- as.tibble(read.csv('tweets_NFT_final.csv'))
tweets30 <- as.tibble(read.csv('tweets_ETH_final.csv'))
etherPrices <- as.tibble(read.csv('etherprices.csv'))
...
freq30 = freq30[-1:-2]
plot_freq30 = ggplot(freq30, aes(x=feature,y=frequency, fill = ifelse(feature == "nft", "highlighted","Normal"))) +
  geom_bar(stat = 'identity') +
  ggtitle("Figure 9: Words Frequently Associated with ETH") +
...
**Figure 10:** Daily ETH Tweet Sentiment and Price (November 2021)
```{r, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
...
plotETH1 = ggplot(sentiments30, aes(x = Dates)) +
  geom_line(aes(y = sentiment, color = "Daily Difference in Sentiment")) +
  geom_line(data = etherPrices1, aes(x = Date, y = (Close.Last - 4000) / coeff, color = "Daily ETH Price")) +
  scale_y_continuous(
    name = "Difference in ETH Sentiment",
    sec.axis = sec_axis(~. *coeff + 4000, name = "Price of ETH (USD)")) +
  theme(text = element_text(size = 10))


#sentiment30_plot = ggplot(sentiments30, aes(x = Dates)) +
#  geom_line(aes(x = Dates, y = sentiment)) +
#  ggtitle("Daily ETH Tweet Sentiment") +
#  labs(x = "Date",
#       y = "Difference in Sentiment Word Frequency") +
#  theme(text = element_text(size = 10))
...
More insightful results can be seen (in Figure 10 above) with the tweet sentiments’ relation to the price of ETH in the same period. The difference in positive and negative ETH sentiment is displayed and appears to be largely associated with given spikes or declines in ETH price. For example, the increase in price to $4800 on November 8th is in sync with significant increase in sentiments on the same day. The crash in price that occurred after on November 14th was quickly followed by a large reduction in positive sentiments. This trend similarly occurs following the ETH boom at the end of November. Thus, the reactionary terms observed in our frequency plot above are supported through this trend and can be interpreted as a call to take action on ETH at a particular instant. Additionally, there is substantial variability of positive sentiment in the data, which may indicate that “verified” users are more likely to tweet positively about ETH in boom periods and not exhibit any sentiment when the price begins to decline. This could potentially be due to a substantial increase in positive headlines concerning ETH in times of boom, which in turn creates similar sentiment through replies. This follows the general reactionary thesis, as such tweets and conversation would naturally follow any metrics observed in previous days.
...
To explore how NFTs are associated with the price of Ethereum, Twitter data was collected for a period of 34 days (July 6 - August 10, 2021) in which the ETH price displayed a significant incline. Filters were similarly applied for English tweets from “verified” accounts, but this time “NFT” was used as a keyword and “giveaways” were removed from the data. This ensured that promotional spam was not included in the limited supply of tweets, due to its significant presence on Twitter and relative unlikelihood of affecting ETH price. After cleaning the data, the number of NFT tweets per day was plotted against the price of ETH, along with a sentiment analysis (shown in Figure 11 below).

The number of NFT tweets appears to be extremely volatile, moving through four distinguishable cycles with lows of 150 to highs of 400 NFT tweets per day. Demonstrating no significant association with the price of ETH, the number of NFT tweets per day seems to be influenced by other factors; perhaps the specific day of the week has an effect on tweet volume, with more users having time to tweet and buy NFTs on weekends.

In contrast, the change in sentiment data on NFTs (found in Figure 12 below) suggests that this asset may be somewhat associated with the price of ETH. The pattern observed in the graph indicates a strong change in sentiment as the price of ETH changes; however, the positive association presented in the ETH sentiment analysis above is not as strong here. This follows a more volatile pattern, where even a slight increase or decrease in the price of ETH may be followed by widely changing sentiments, as observed on July 12th and 26th. The higher volatility can potentially be explained by NFT Twitter data occurring in smaller magnitudes than ETH Twitter data in a similar time range. However, a more interesting point demonstrated in this graph, is that the ETH price seems to almost be reactionary to the NFT sentiment! An example can be observed on July 18th, where a large increase in NFT sentiment is followed by gains in ETH price, starting on July 19th. Since NFTs are one of, if not, the main asset on the ETH blockchain, this data makes sense and could prove to be valuable when predicting ETH price.

\pagebreak

**Figure 11:** Daily NFT Tweet Number and ETH Price (July 5 - August 10, 2021)
```{r, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
...
plotETH2 = ggplot(tweetsPerDay, aes(x = Dates)) +
  geom_line(aes(y = tweetsPerDay1, color = "Number of tweets")) +
  geom_line(data = etherPrices2, aes(x = Date, y = (Close.Last - 1500) / coeff2, color = "Daily ETH Price")) +
  scale_y_continuous(
    name = "Number of NFT Tweets per Day",
    sec.axis = sec_axis(~. *coeff2 + 1500, name = "Price of ETH (USD)")) +
  theme(text = element_text(size = 10))
...
**Figure 12:** Daily NFT Tweet Sentiment and ETH Price (July 5 - August 10, 2021)
```{r, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
...
plotNFT = ggplot(sentiments, aes(x = Dates)) +
  geom_line(aes(y = sentiment, color = "Daily Difference in NFT Sentiment")) +
  geom_line(data = etherPrices2, aes(x = Date, y = (Close.Last - 1600) / coeff3, color = "Daily ETH Price")) +
  scale_y_continuous(
    name = "Difference in NFT Sentiment",
    sec.axis = sec_axis(~. *coeff3 + 1600, name = "Price of ETH (USD)")) +
  theme(text = element_text(size = 10))

plotNFT

```
Ultimately, the results imply there may be an association between the price of ETH and NFT sentiment, supporting the frequency plot’s results that the term “nft” is important to ETH. Similar to the ETH data above, NFTs’ newfound popularity on social media platforms and unique qualitative application to the Ethereum blockchain could be valid reasons for associating this topic to future ETH price changes, especially as its market size continues to grow in time.
